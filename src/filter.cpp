...
void Filter::findMagnitude(CB& a,double* mag)
{
    for(size_t i = 0; i < a.size(); i++)
    {
        mag[i] = sqrt(pow(a[i].x,2) + pow(a[i].y,2) + pow(a[i].z,2) );
    }
}

void Filter::findFinal(double* mag,double* meanMag)
{
    
    movingAverage(mag,meanMag,maxSize,WINDOW);
    for(int i=0; i < maxSize; i++)
    {
        meanMag[i] = mag[i] - meanMag[i]; 
    }
}